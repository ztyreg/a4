...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # initialize to 0
        first_hidden = torch.zeros([2, source_padded.size()[1], self.hidden_size])
        first_cell = torch.zeros([2, source_padded.size()[1], self.hidden_size])
        # pack X
        X = nn.utils.rnn.pack_padded_sequence(X, source_lengths)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X, (first_hidden, first_cell))
        # pad enc_hiddens
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # torch.cat(tensors, dim=0, out=None) → Tensor
        U_t = torch.cat((torch.squeeze(a_t, dim=1), torch.squeeze(dec_hidden, dim=2)), dim=1)
